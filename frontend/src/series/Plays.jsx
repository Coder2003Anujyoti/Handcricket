...
{ (val[0].matches.filter((i)=> i.winner == "").length > 0 ) && <button onClick={() => setMode("play")}
className={`px-4 py-2 font-bold  ${mode === "play" ? 'border-b border-b-white text-white' : 'text-white border-b border-b-transparent'}`}>Play 
  </button> }
 {(val[0].matches.filter((i)=> i.winner == "").length > 0 ) && <button onClick={() => setMode("matches")}
...
{(val[0].matches.filter((i)=> i.winner == "").length == 0) && <button onClick={() => setMode("result")}
...
<div className="w-72 h-52 bg-slate-800 mt-12 mb-6 flex flex-row justify-center items-center rounded-md">
{ val[0].contestants.filter((it)=> it.name == name).map((i)=>{
  return(<>
...
<p className="text-center text-sm text-white font-bold">Match {4-val[0].matches.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").length}</p>
...
<HashLink smooth to={`/onlines?matchtype=${encodeURIComponent(`Match ${4-val[0].matches.filter((i)=> (i.firstteam.name == name || i.secondteam.name== name) && i.winner == "").length}`)}&&id=${encodeURIComponent(name)}&&player=${encodeURIComponent(i.firstteam.name)}&&playerteam=${encodeURIComponent(i.firstteam.team)}&&computerteam=${encodeURIComponent(i.secondteam.team)}&&matchID=${encodeURIComponent(id)}`}>
...
<img src={`Logos/${teamicons.filter((i)=> i.team == chart.slice().sort((a,b)=> b.win - a.win)[0].team)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-white font-bold">{val[0].contestants.filter((i)=> i.team == chart.slice().sort((a,b)=> b.win - a.win)[0].team)[0].name}</p>
</div>
<div className="flex justify-center items-center flex-col gap-2">
<p className="text-yellow-400 text-xs font-bold">Loser</p>
<img src={teamicons.filter((i)=> i.team == chart.slice().sort((a,b)=> a.win - b.win)[0].team)[0].image} className="w-24 h-24" />
<img src={`Logos/${teamicons.filter((i)=> i.team == chart.slice().sort((a,b)=> a.win - b.win)[0].team)[0].team}.webp`} className="w-8 h-8" />
<p className="text-center text-xs text-white font-bold">{val[0].contestants.filter((i)=> i.team == chart.slice().sort((a,b)=> a.win - b.win)[0].team)[0].name}</p>
</div>
</div>
  </>
}
