...
useEffect(()=>{
  ...
  useEffect(() => {
    socket.on('dualwaits', (message) => {
    setMsg(message);
  });

  socket.on('dualstartGames', (message) => {
    setStart(true);
    setMsg("")
    setData(message);
  });
  socket.on('dualchoiceturns',(ms)=>{
    if(ms=="Your Turn"){
    setMsg(ms)
    setOpt(0)
  ...
  return () => {
    socket.off('dualwaits')
    socket.off('dualLefts')
...
const add_Name=(i,it)=>{
  if (disable==false) {
  socket.emit('dualjoinRooms', {name:id,team:i,player:it,matchID,matchtype,teamone:idteam,teamtwo:adminteam});
    setDisable(true)
    }
}
const optio=(i)=>{
  if(opt==0){
   socket.emit('dualgomoves',i)
  clearInterval(countdownInterval.current);
  clearTimeout(inactivityTimeout.current);
  setOpt(i)
  }
